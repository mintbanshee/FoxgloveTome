...
// Determine the action to take based on the 'action' parameter in POST or GET, default to 'login' if not provided
$action = filter_input(INPUT_POST, 'action') ?? filter_input(INPUT_GET, 'action') ?? 'login'; 

    switch ($action) {

    // signup and default to user role 
    case 'signup':
        $errors = [];
        $email = '';
        $first = '';
        ...
        if (empty($errors)) {
            ...
            if ($statement->fetch()) {
                ...
                // create the user in the database and hash the password for security
    ...
    case 'login':
        ...
        if (!$user || !password_verify($password, $user['password_hash'])) {
            ...
            // for additional security, if login successful create a fresh session id 
            session_regenerate_id(true); 

            $name = trim(($user['first_name'] ?? '') . ' ' . ($user['last_name'] ?? ''));

            // create the user session with their information
            $_SESSION['user'] = [
            ...
            // if admin send to user dashboard but there will be a button only admin sees to access admin dashboard from there
    ...
    case 'logout':
        ...
        // clear the session data and destroy the session cookie for security
        $_SESSION = []; 
    ...
    case 'forgot_password':
        ...
        case 'reset_password':
            ...
            // if the form is submitted, validate and update the password
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
