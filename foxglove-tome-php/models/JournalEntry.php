...
class JournalEntry {
  ...
  public function __construct() {
    ...
    // Assign the PDO connection to the class property
    $this->conn = $pdo;
  }
  ...
  public function getWeeklyMoodSummary(int $userID): array {
    ...
    $oneWeekAgo = strtotime(date('Y-m-d', strtotime('-7 days'))); 
    ...
    foreach ($entries as $entry) {
        ...
        // only continue if there are moods to count
        if (!empty($moodCounts)) { 
          // sort by count
            arsort($moodCounts); 
        ...
        // which category does the dominant mood belong to
            foreach ($weeklyEntries as $entry) { 
                if ($entry['mood'] === $dominantMood) { 
                    $dominantCategory = $entry['mood_category']; 
                  break; 
                }
            }
